...
def scaled_dot_product_attention(q, k, v, mask):
    ...
    # 乘以value
    # shape=[batch_size, seq_len_q, depth]
...
class Encoder(tf.keras.layers.Layer):
    def __init__(self, d_model, num_layers, num_heads, dff,
        ...
        self.pos_encoding = positional_encoding(
            max_seq_len, d_model)  # shape=[1, max_seq_len, d_model]
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, encoder_out, training, look_ahead_mask, padding_mask):
        ...
        att_out1, att_weight1 = self.mha1(
        ...
        att_out2, att_weight2 = self.mha2(
            att_out1, encoder_out, encoder_out, padding_mask)
...
class Transformer(tf.keras.Model):
    def __init__(self, d_model, num_layers, num_heads, dff,
        ...
        self.encoder = Encoder(d_model, num_layers, num_heads,
                               dff, input_vocab_size, max_seq_len, dropout_rate)
        self.decoder = Decoder(d_model, num_layers, num_heads,
                               dff, target_vocab_size, max_seq_len, dropout_rate)
    ...
    def call(self, inputs, targets, training, encoder_padding_mask,
        ...
        decoder_output, att_weights = self.decoder(targets, encoder_output, training,
                                                   look_ahead_mask, decoder_padding_mask)
        # 最后映射到输出层
        # shape=[batch_size, seq_len_target, target_vocab_size]
